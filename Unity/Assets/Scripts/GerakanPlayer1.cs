...
public class GerakanPlayer1 : MonoBehaviour
{
    ...
    void HandleGrab()
    {
        if (grabbedObject == null)
        {
            ...
            if (Physics.Raycast(ray, out hit, grabDistance))
            {
                // 1. CEK DISSOLVING ITEM
                DissolvingItem dItem = hit.collider.GetComponent<DissolvingItem>();
                if (dItem != null)
                {
                    if (dItem.tipeBarang == DissolvingItem.ItemType.Dokumen)
                        ScoreUIManager.Instance.AddDokumen();
                    else if (dItem.tipeBarang == DissolvingItem.ItemType.BarangPenting)
                ...
                // 2. CEK HAZARD ITEM
                HazardItem hItem = hit.collider.GetComponent<HazardItem>();
                if (hItem != null)
                {
                    if (hItem.hazardType == HazardItem.HazardType.Elektronik)
                        ScoreUIManager.Instance.AddElektronik();
                    else if (hItem.hazardType == HazardItem.HazardType.ZatBerbahaya)
                        ScoreUIManager.Instance.AddRacun();

                    Destroy(hItem.gameObject);
                    return;
                }
